...
async def test_suggest_returns_top_candidate(engine):
    """End-to-end suggest path: context_text seeds the target; the aligned
    candidate must win over its negation."""
    from datetime import datetime, timezone

    from skynet_vibe import FacetVectors, VibeSignal
    from skynet_vibe import Source as Src

    # A signal aligned with the context vector steers the weighted target.
    ctx_vec = await engine.embedder("evening music")
    sig = VibeSignal(
        id="seed-1",
        text_raw="evening music",
        vectors=FacetVectors(content=list(ctx_vec)),
    ...
    close_vec = list(ctx_vec)
    far_vec = [-x for x in ctx_vec]
    ...
    result = await engine.suggest(
        ...
        domain=None,
        context_text="evening music",
        top_k=2,
    )
    assert result.candidate["id"] == "rec_close"
    assert result.rec_id
    assert result.reason
    # The seed signal should contribute to the pool.
...
@pytest.mark.asyncio
async def test_suggest_proceeds_without_prototype_ready(engine):
    """Engine no longer requires prototypes._ready — suggest() must work
    as long as domain or context_text is supplied."""
    # prototypes._ready is NOT set; engine uses _domain_seed() instead.
    result = await engine.suggest(
        candidates=[{"id": "x", "vector": [0.1] * 16}],
        domain="music",
        context_text="evening",
    )
    assert result.candidate["id"] == "x"
